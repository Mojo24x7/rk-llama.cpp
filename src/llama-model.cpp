        ...
        case LLM_ARCH_QWEN35MOE:
            {
                ...
                // NextN/MTP: some conversions include the multi-token-prediction layer
                // in block_count. It is not evaluated, but its tensors are in the file.
                ml.get_key(LLM_KV_NEXTN_PREDICT_LAYERS, hparams.nextn_predict_layers, false);
                GGML_ASSERT(hparams.nextn_predict_layers < hparams.n_layer && "nextn_predict_layers must be < n_layer");
                ...
                if (hparams.nextn_predict_layers > 0) {
                    // no KV cache for the trailing MTP layers
                    hparams.n_layer_kv_from_start = n_transformer_layers;
                }
                ...
                // Mark recurrent layers (linear attention layers)
                {
                    ...
                    ml.get_key(LLM_KV_FULL_ATTENTION_INTERVAL, full_attn_interval, false);
                    for (uint32_t i = 0; i < hparams.n_layer; ++i) {
                        // MTP layers are full-attention blocks and are never run:
                        // keep them out of the recurrent set so no SSM state is allocated
                        hparams.recurrent_layer_arr[i] = (i < n_transformer_layers) && ((i + 1) % full_attn_interval != 0);
                    }
                }

                switch (n_transformer_layers) {
            ...
            case LLM_ARCH_QWEN35MOE:
                {
                    ...
                    const uint32_t n_mtp_layers = hparams.nextn_predict_layers;

                    for (int i = 0; i < n_layer; ++i) {
                        auto & layer = layers[i];

                        // NextN/MTP layers are shipped in the file but never evaluated.
                        // TENSOR_SKIP claims them (so the tensor count matches) without
                        // allocating or reading their data; NOT_REQUIRED tolerates builds
                        // that omit some of them.
                        const bool is_mtp = n_mtp_layers > 0 && (uint32_t) i >= (uint32_t) n_layer - n_mtp_layers;
                        ...
                        if (!hparams.is_recurrent(i)) {
                            ...
                            // Linear attention (gated delta net) specific tensors
                            // Create tensors with calculated dimensions
                            layer.wqkv           = create_tensor(tn(LLM_TENSOR_ATTN_QKV,       "weight", i), { n_embd, key_dim * 2 + value_dim }, flags | TENSOR_NOT_REQUIRED);
                            layer.wqkv_gate      = create_tensor(tn(LLM_TENSOR_ATTN_GATE,      "weight", i), { n_embd, value_dim }, flags | TENSOR_NOT_REQUIRED);
                        ...
                        if (is_mtp) {
                            // NextN/MTP tensors (claimed but unused)
                            ...
                            create_tensor(tn(LLM_TENSOR_NEXTN_SHARED_HEAD_NORM,  "weight", i), { n_embd }, flags);
                            create_tensor(tn(LLM_TENSOR_NEXTN_EMBED_TOKENS,      "weight", i), { n_embd, n_vocab }, flags);
                            create_tensor(tn(LLM_TENSOR_NEXTN_SHARED_HEAD_HEAD,  "weight", i), { n_embd, n_vocab }, flags);
                        }
                    }
                } break;
            case LLM_ARCH_QWEN35:
