...
bool llama_model_base::load_tensors(llama_model_loader & ml) {
    ...
    // kernel for that layer's matmuls - a large net loss (+84% decode on
    // Qwen3.6-35B-A3B / RK3588 when recurrent layers stay on the CPU).
    ...
    auto get_layer_buft_list = [&](int il) -> llama_model::impl::layer_dev {
        ...
        if (recurrent_on_cpu && il < n_layer_all && hparams.is_recr(il)) {
